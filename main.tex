...
% Ondorio nagusiak:
% Euskararako bert propioa > mbert publikoa eta euskararako FastText propioa > FastText publikoa eta flair propioa > flair publikoa
% Atazak: topic class / polarity / ner / pos
% Artearen egoera ataza guztietan (konprobatu bakoitzak berea)
% Euskaraz: bert > flair > FastText (ñabardurekin idatzi)
...
\hline
\end{tabular}
\caption{Basque topic classification results on Argia corpus.}\label{tab:corpus}
\end{table}
...
\todo[inline]{Hemen gehitu dut seedaren komentarioa, ez dakit leku onena zen}

Every reported result for every system is the average of five randomly initialized runs. As we use the code developed by \newcite{Wolf2019HuggingFacesTS} in PoS and NER, we remove the seed for random initialization. 
...
BERT-BMC & \textbf{97.68}/97.76 \\ \hline
\textbf{Baseline} \\
...
\todo[inline]{/ eta gero gehitutakoak (PoS eta NERen) hugginface-n zuzenean lortzen direnak dira (seed gabe 5 random run eginez). Hauek utziko ditugu?}
\begin{table}[!ht]\footnotesize
\centering
\begin{tabular}{@{\hspace{0.3cm}}lccc} \hline
...
BERT-BMC  & 88.11/87.95 & 87.11/86.11 & \textbf{87.61}/87.06 \\ \hline
\textbf{Baseline} \\
...
\textbf{GS: }      & B-LOC & O & O & O & O & O & \textbf{B-ORG} & \textbf{I-ORG} & O & \textbf{B-ORG} & \textbf{I-ORG }& O & O & O. \\
\textbf{mBERT} & B-LOC & O & O & O & O & O & \textbf{B-LOC} & \textbf{I-LOC} & O & \textbf{B-LOC} & \textbf{I-LOC} & O & O & O. \\
\textbf{BERTeus} & B-LOC & O & O & O & O & O & \textbf{B-ORG} & \textbf{I-ORG} & O & \textbf{B-ORG} & \textbf{I-ORG} & O & O & O. \\
\end{tabular}}
